...
export default defineComponent({
  ...
  mounted() {
    this.$nextTick(() => {
      this.container = document.getElementById('suckerContainer');
      const { top, left, width, height } = this.suckerCanvas.getBoundingClientRect();
      this.top = top;
      this.left = left;
      this.width = width;
      this.height = height;
  ...
  methods: {
    ...
    mousemoveHandler(e: any) {
      if (!this.isOpenSucker) return
      const { clientX, clientY } = e

      const x = Math.min(clientX - this.left, this.width - 1);
      const y = Math.min(clientY - this.top, this.height - 1);
      ...
      Object.assign(style, {
        position: 'absolute',
        left: clientX - (this.previewSize / 2) + 'px',
        top: clientY - (this.previewSize / 2) + 'px',
        zIndex: 95, // The level of the preview color of the small circle of the eyedropper cannot exceed the color selector
      })
      if (clientX >= this.left && clientY >= this.top && clientX <= (this.left + this.width)
